    ...
    <div className="SolidExportProgressToaster">
     <div className="flex align-items-center justify-content-between gap-3">
        ...
        <div className="flex-1">
          <h2 className={`text-lg font-semibold m-0 ${status === "error" ? "text-red-500" : "pExportText"}`}>
            {message}
          </h2>
          <p className={`text-sm ${status === "error" ? "text-red-400" : "pExportText"}`}>
            {submessage}
          </p>
        </div>
        {status === "success" && (  
      ...
      {status !== "error" && (
        <ProgressBar value={progress} showValue={false} className="exportCustomProgressbar" />
      )}
    </div>
  );
};
