...
import {
  useRelationEntityHandler,
  RelationItem,
} from "./widgets/helpers/useRelationEntityHandler";
 ...
 */
export const DefaultRelationManyToManyAutoCompleteFormEditWidget = ({
  formik,
  fieldContext,
  ...
  const autoCompleteSearch = async (event: AutoCompleteCompleteEvent) => {
    ...
    if (
      fieldMetadata?.relationFieldFixedFilter ||
      fieldLayoutInfo?.attrs?.whereClause
    ) {
      const rawFilter =
        fieldLayoutInfo?.attrs?.whereClause ??
        fieldMetadata?.relationFieldFixedFilter;
  ...
  const isUnsaved =
    fieldContext.data?.id === undefined || fieldContext.data?.id === "new";
  const entityName =
 ...
 */
export const DefaultRelationManyToManyCheckBoxFormEditWidget = ({
  formik,
  fieldContext,
}: SolidFormFieldWidgetProps) => {
  const fieldMetadata = fieldContext.fieldMetadata;
      ...
      <SolidPanel>
        <div className="flex flex-wrap -mx-2 -mt-2">
          {allOptions.map((item: any, i: number) => (
            <div
              key={item.value}
              className={`field flex w-full gap-2 sm:w-1/2 ${i >= 2 ? "mt-1 md:mt-3" : "mt-1 md:mt-0"}`}
            >
              <SolidCheckbox
...
export const DefaultRelationManyToManyListFormEditWidget = ({
  formik,
  fieldContext,
}: SolidFormFieldWidgetProps) => {
  const fieldMetadata = fieldContext.fieldMetadata;
  ...
  const [visibleCreateRelationEntity, setvisibleCreateRelationEntity] =
    useState(false);
  ...
  const [
    showSaveParentEntityConfirmationPopup,
    setShowSaveParentEntityConfirmationPopup,
  ] = useState(false);
  ...
  const { fetchSuggestions, linkItem, unlinkItem, suggestions } =
    useRelationEntityHandler({ fieldContext });
  ...
  const handleEditClickForEmbeddedView = (id: any) => {
    if (id === "new") {
      ...
      setformViewParams({
        ...
        modelName: camelCase(
          fieldContext.fieldMetadata.relationCoModelSingularName,
        ),
        parentFieldName: fieldContext.fieldMetadata.relationCoModelFieldName,
        parentData: userKeyField
          ? {
              [userKeyField]: {
                solidManyToOneLabel: fieldContext.data[userKeyField],
                solidManyToOneValue: fieldContext.data["id"],
              },
            }
          : {},
      ...
      <div className={styles.fieldWrapper}>
        ...
        {listViewParams && (
          <SolidListView
            ...
            {...listViewParams}
            embededFieldRelationType="many-to-many"
            handleAddClickForEmbeddedView={handleAddClickForEmbeddedView}
            handleEditClickForEmbeddedView={handleEditClickForEmbeddedView}
      ...
      >
        ...
        <SolidDialogBody>
          <div className="flex flex-col gap-2 pt-2">
            <label className="form-field-label">Search {fieldLabel}</label>
            <SolidAutocomplete
              field="label"
...
export const DefaultRelationManyToManyFormViewWidget = ({
  formik,
  fieldContext,
}: SolidFormFieldWidgetProps) => {
  const fieldMetadata = fieldContext.fieldMetadata;
  ...
  const { currentValues, fetchCurrentValues } = useRelationEntityHandler({
    fieldContext,
  });
