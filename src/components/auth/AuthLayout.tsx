...
export const AuthLayout = ({ children }: { children: React.ReactNode }) => {
    ...
    return (
        ...
        <div className={`solid-auth-theme-wrapper ${authLayout} auth-theme-${authTheme}`} data-auth-theme={authTheme}>
            ...
            <div className={`absolute hidden md:flex ${solidSettingsData?.data?.authPagesLayout === 'center' ? 'solid-auth-footer flex flex-col sm:flex-row items-center justify-between' : 'solid-auth-footer-2 flex flex-wrap'}`}>
                {solidSettingsData?.data?.authPagesLayout !== 'left' &&
                    <div className={solidSettingsData?.data?.authPagesLayout !== 'center' ? 'w-1/2 lg:w-[41.666667%] xl:w-1/2 flex justify-center' : ''}>
                        {solidSettingsData?.data?.showLegalLinks === true &&
                ...
                <div className={solidSettingsData?.data?.authPagesLayout !== 'center' ? 'w-1/2 flex justify-center' : ''}>
                    {solidSettingsData?.data?.showLegalLinks === true && <div className={`flex flex-col sm:flex-row items-center gap-1 sm:gap-5 solid-auth-subtitle mr-4 ${solidSettingsData?.data?.authPagesLayout === 'left' ? 'left' : ''}`}>
